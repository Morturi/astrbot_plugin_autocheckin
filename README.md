...
2. 先输入 WebUI 登录密钥（`webui_token`）完成登录验证
...
8. 将录制的操作保存到对应站点
9. 登录后若切换访问 IP，或超过 `webui_session_timeout` 配置的时间未进行操作，需要重新输入登录密钥
...
| `webui_session_timeout` | int | 30 | WebUI 登录超时（分钟），超时未操作需重新登录 |
...
- WebUI 通过 `webui_token` 登录，并在切换访问 IP 或超过 `webui_session_timeout` 未操作时要求重新登录。
...
- 已为 WebUI 增加 token 登录鉴权，并支持按配置的会话超时重新登录。
